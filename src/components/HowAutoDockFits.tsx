...
const HowAutoDockFits = () => {
  ...
  return <section className="section-padding bg-gradient-surface relative overflow-hidden" id="how-autodock-fits">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative max-w-7xl mx-auto" onClick={handleClickOutside}>
            ...
            zIndex: 1
              ...
              {/* Top arc */}
              <path d="M 200 120 Q 512 80 824 120" stroke="url(#gradient1)" strokeWidth="2" fill="none" className="opacity-40" />
              {/* Right side */}
              <path d="M 824 120 Q 864 200 824 280" stroke="url(#gradient2)" strokeWidth="2" fill="none" className="opacity-40" />
              {/* Bottom arc */}
              <path d="M 824 280 Q 512 320 200 280" stroke="url(#gradient3)" strokeWidth="2" fill="none" className="opacity-40" />
              {/* Left side */}
              <path d="M 200 280 Q 160 200 200 120" stroke="url(#gradient4)" strokeWidth="2" fill="none" className="opacity-40" />
            ...
            <div className="flex items-center justify-center mb-20 relative" style={{
            zIndex: 2
          }}>
                ...
                <div className="flex items-center justify-center p-10 h-full">
                  <div className="text-center space-y-6">
                    ...
                    <div className="flex justify-center items-center space-x-4 pt-6">
                      {steps.map((_, index) => <div key={index} className={`w-4 h-4 rounded-full transition-all duration-500 ${highlightedStep === index ? 'bg-primary scale-125 shadow-lg shadow-primary/40' : 'bg-primary/20 hover:bg-primary/40'}`} />)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              return <div key={index} className={`absolute ${getPositionClasses(step.position)} z-10`} style={{
                animationDelay: `${0.3 + index * 0.1}s`,
                animationFillMode: 'both'
              }}>
                    {/* Step Card */}
                    <div className={`group cursor-pointer relative animate-fade-in transition-all duration-300 ${isHighlighted ? 'scale-110' : 'hover:scale-105'}`} onClick={e => handleStepClick(index, e)} onMouseEnter={() => setActiveStep(index)} onMouseLeave={() => {
                  if (timeoutRef.current) {
                    clearTimeout(timeoutRef.current);
                  }
                  timeoutRef.current = setTimeout(() => setActiveStep(null), 200);
                      ...
                      <div className={`bg-background/95 backdrop-blur-sm rounded-2xl p-5 lg:p-6 border shadow-lg hover:shadow-xl transition-all duration-300 w-52 lg:w-64 ${isHighlighted ? 'border-primary/50 shadow-primary/25 ring-1 ring-primary/20' : 'border-border/40 hover:border-primary/40 hover:shadow-primary/10'}`}>
                        <div className="flex items-center space-x-4">
                          {/* Icon with red number badge */}
                          <div className="relative flex-shrink-0">
                            <div className={`w-14 h-14 lg:w-16 lg:h-16 bg-gradient-to-br from-primary via-primary to-primary/90 rounded-xl flex items-center justify-center transition-all duration-300 ${isHighlighted ? 'shadow-lg shadow-primary/40 scale-110' : 'shadow-md'}`}>
                          ...
                          <div className="min-w-0 flex-1">
                            <h3 className={`body-base font-bold transition-colors ${isHighlighted ? 'text-primary' : 'text-foreground group-hover:text-primary'}`}>
                              {step.title}
                            </h3>
                          </div>
                          
                          {/* Arrow indicator */}
                          <ArrowRight className={`w-5 h-5 text-muted-foreground transition-all duration-300 ${isActive ? 'rotate-90 text-primary' : 'group-hover:text-primary group-hover:translate-x-1'}`} />
                        </div>
                      </div>

                      {/* Tooltip/Pop-up for description */}
                      <div className={`absolute ${getTooltipPosition(step.position)} transition-all duration-300 z-20 ${isActive ? 'opacity-100 translate-y-0 scale-100 pointer-events-auto' : 'opacity-0 translate-y-3 scale-95 pointer-events-none'}`}>
                        <div className="bg-background/98 border border-border/60 rounded-2xl shadow-2xl p-6 max-w-sm backdrop-blur-sm ring-1 ring-primary/10">
                          ...
                          {/* Arrow pointer */}
                          <div className={`absolute w-4 h-4 bg-background border-l border-t border-border/60 transform rotate-45 ${step.position.includes('top') ? '-bottom-2 left-6' : step.position.includes('bottom') ? '-top-2 left-6' : step.position.includes('left') ? 'top-1/2 -translate-y-1/2 -right-2' : 'top-1/2 -translate-y-1/2 -left-2'}`}></div>
                        </div>
                      </div>
                    </div>
                  </div>;
